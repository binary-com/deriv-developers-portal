...
export default function HomepageSlider() {
  ...
  const [enable_slide_animation, setEnableSlideAnimation] =
    React.useState(true);
  ...
  const FirstSlide = () => {
    return (
      <Slide
        content={
          <span>
            To be honest, Deriv's API is one of the best APIs in the trading
            market.
          </span>
        }
        author={
  ...
  const LastSlide = () => {
    return (
      <Slide
        content={
          <span>
            I have been using the Deriv API for 13 years to build successful
            apps in and I find the support I get from Deriv as a business
            partner second to none. I look forward to 13 more successful years
            to come.
          </span>
        }
        author={
  ...
  const slideCard = (mouse_x: number) => {
    ...
    if (!enable_slide_animation && is_holding_card)
      setEnableSlideAnimation(true);
    if (slide_right) {
      setIsHoldingCard(false);
      slidingTo(PREVIOUS);
    } else if (slide_left) {
