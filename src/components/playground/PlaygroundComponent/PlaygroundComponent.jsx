...
export const PlaygroundComponent = () => {
  ...
  useEffect(() => {
    if (window.location.hash) {
      const hash_value = window.location.hash.split("#")[1];
      const find_select_value = playground_requests.find(
        (el) => el.name === hash_value
      );
  ...
  const sendRequest = useCallback(() => {
    ...
    try {
      _request =
        request_input.current?.value &&
        JSON.parse(request_input.current?.value);
    } catch (error) {
      alert("Invalid JSON!");
      return;
    }
    ...
    const subscribed_tick_history =
      _request.ticks_history && _request.subscribe === 1;
